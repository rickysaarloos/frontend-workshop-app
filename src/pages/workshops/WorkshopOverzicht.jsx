...
// 🔧 Vervang met: const workshops = await getWorkshops()
// ============================================================
const mockWorkshops = [
...
/**
 * Geeft alle dagen terug die in de kalenderweergave zichtbaar zijn voor een maand.
 */
...
/**
 * Workshop overzichtspagina met kalender en lijst naast elkaar.
 *
 * @returns {JSX.Element}
 */
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="max-w-5xl mx-auto w-full px-4 py-8">
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <h1 className="text-2xl font-bold text-[#1a3d2b] tracking-tight">Workshops</h1>
          <p className="text-sm text-gray-400 mt-1">Kies een workshop om je voor in te schrijven</p>
        </motion.div>
        ...
        <div className="flex gap-6 items-start">
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 w-72 shrink-0"
          >
            {/* Maand navigatie */}
            <div className="flex items-center justify-between mb-4">
              <motion.button
                ...
                className="p-1.5 rounded-xl hover:bg-gray-50 text-gray-400 hover:text-[#1a3d2b] transition-colors"
              >
              ...
              {kalenderDagen.map((dag, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`relative aspect-square flex items-center justify-center rounded-xl text-xs font-medium transition-all duration-150 mx-auto w-8 h-8
                      ${isGeselecteerd
                        ? 'bg-[#1a3d2b] text-[#d4e84a] font-bold shadow-md'
                        : isVandaag
                          ? 'bg-[#d4e84a] text-[#1a3d2b] font-bold'
                          : heeftWorkshop
                            ? 'text-[#1a3d2b] hover:bg-[#eaf3de]'
                            : 'text-gray-400 hover:bg-gray-50'
                      }`}
                  >
                    {dag}
                    {/* Stipje voor workshops */}
            ...
            <div className="mt-4 pt-4 border-t border-gray-50 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 text-xs text-gray-400">
                <span className="w-5 h-5 rounded-lg bg-[#d4e84a] shrink-0 flex items-center justify-center text-[10px] font-bold text-[#1a3d2b]">•</span>
                Vandaag
              </div>
              {geselecteerdeDag && (
                <motion.button
                  ...
                  className="mt-1 text-xs text-[#1a3d2b] font-semibold hover:underline underline-offset-2 text-left"
                >
                  ✕ Filter wissen
                </motion.button>
              )}
              ...
              >
                {zichtbareWorkshops.length === 0 ? (
                  <div className="bg-white rounded-3xl border border-gray-100 p-8 text-center">
                    ...
                    <button onClick={() => setGeselecteerdeDag(null)} className="mt-2 text-xs text-[#1a3d2b] font-semibold hover:underline">
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {isVol && (
                                <span className="bg-red-50 text-red-400 text-xs font-semibold px-2 py-0.5 rounded-lg border border-red-100">
                                  vol
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3 text-xs text-gray-400 mb-3">
                              <span className="flex items-center gap-1">
                                ...
                                <span className="capitalize">{formatDatum(workshop.datum)}</span>
                              </span>
                              <span className="flex items-center gap-1">
                                <span className="text-[#1a3d2b]">🕐</span>
                                {workshop.tijd}
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex-1 bg-gray-100 rounded-full h-1.5">
                                <div
                                  className={`h-1.5 rounded-full transition-all ${isVol ? 'bg-red-300' : 'bg-[#1a3d2b]'}`}
                                  style={{ width: `${procentVol}%` }}
                                />
                              </div>
                              <span className={`text-xs font-semibold shrink-0 ${isVol ? 'text-red-400' : 'text-[#1a3d2b]'}`}>
